...
(function(exports) {
  ...
  function Manana(view_dir) {
    ...
    this.interpreter.If = function(form, context) {
      var body, num_branches, branch, last_branch, next_branch, branch_stop, 
          num_exprs, exprs, expr, next_expr, i, j, op, v1, v2, res,
          _has_else, _negate;
      ...
      while (i < branch_stop) {
        branch      = body[i];
        next_branch = body[i+1];
        exprs       = branch.expressions;
        expr_stop   = exprs.length;
        op          = branch.op;
        _negate     = branch.negate;

        console.log('branch: ', branch);
        console.log('next_branch: ', next_branch);
        ...
        while (j < expr_stop) {
          ...
          switch (op) {
          case true:
            res = !!self.interpreter.evalForm(v1, context);
            break;
          case '==':
            break;
          case '!=':
