...
(function(exports) {
  ...
  function Manana(config) {
    ...
    function MananaAncestryRecord(view) {
      ...
      this.snapshot = typeof Object.create !== "undefined"
        ? this.snapshot = Object.create(view)
    ...
    // ........................................... 
    /**
     * Mañana Chronology Record
     * @class MananaChronologyRecord
     * @param {MananaView} view - The Mañana view to be recorded.
     */
    function MananaChronologyRecord(view) {
      if (!(view instanceof MananaView)) {
        self.err = new MananaError('You must pass an instance of MananaView to Manana Chronology Record', view);
        throw self.err;
      }
    ...
    this.interpreter.Include = function(form, context, level) {
      ...
      // Add to family.
      if (self.view_level < self.history.family.length) {
        self.history.family = self.history.family.slice(0, self.view_level);
      }
    ...
    // ...........................................  
    /**
     * Quick historical record lookup.
     * @memberof Manana
     * @type {Array}
     * @namespace Manana.history
     */
    this.history.chronology = [];
    ...
    this.history.add = function(view) {
      var limit, ancestry, chrono;

      if (!(view instanceof MananaView)) {
        self.err = new MananaError('Invalid argument to History Set. Must be MananaView instance.', view);
        throw self.err;
      }

      limit    = self.options.history.limit;
      ancestry = self.history.ancestry;
      chrono   = self.history.chronology;

      if (ancestry.length >= limit) ancestry.shift();
      ancestry.push(new MananaAncestryRecord(view));

      if (chrono.length >= limit) chrono.shift();
      chrono.push(new MananaChronologyRecord(view));
